...
window.demoDescription =
  "A series of points forming a fibonacci spiral disc, distorted by mouse position, and connected as delaunay triangles.";
...
function fibonacci(num, scale, offset, smooth) {
  ...
  for (var i = 0; i < num; i++) {
    ...
    pts.push(
      new Vector(r * scale * Math.cos(theta), r * scale * Math.sin(theta)).add(
        offset
      )
    );
  }
