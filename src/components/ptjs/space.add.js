...
window.demoDescription =
  "A very simple demo of Pt's main concepts: extending Pt classes, drawing with Form, and animating in Space.";
...
function Dust() {
  ...
  this.color =
    this.weight > 0.7 ? colors["a" + Math.ceil(Math.random() * 4)] : "#000";
}
...
Dust.prototype.animate = function (time, fps, context) {
  ...
  gray = Math.max(
    0,
    Math.min(0.6, Math.random() > 0.5 ? gray + 0.05 : gray - 0.05)
  );
